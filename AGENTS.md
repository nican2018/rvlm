...
This guide covers best practices for contributing to the core Recursive Language Models `rvlm` library and developing new environments (in `rvlm/environments/`) and LM clients (in `rvlm/clients/`).

## Setup

### Option 1: Conda (Recommended for RVLM)
We provide an `environment.yml` for easy setup with Conda, including dependencies for `rvlm` and common medical imaging tools.
```bash
...
For PRs to `rvlm` core:
```bash
git clone https://github.com/alexzhang13/rvlm.git
cd rvlm
...
LM client implementations live in `rvlm/clients/`. All clients must inherit from `BaseLM`.
...
### Requirements
- Inherit from `BaseLM` in `rvlm/clients/base_lm.py`
...
- Register client in `rvlm/clients/__init__.py`
...
Environment implementations live in `rvlm/environments/`. Choose the appropriate base class.
...
### Requirements
- Inherit from `NonIsolatedEnv` or `IsolatedEnv` in `rvlm/environments/base_env.py`
...
- Register environment in `rvlm/environments/__init__.py`
...
### Example Structure
```python
from rvlm.environments.base_env import NonIsolatedEnv
from rvlm.core.types import REPLResult
...
```python
# Sending a message (from rvlm/core/comms_utils.py)
...
**Key Components**:
- `LMHandler` (`rvlm/core/lm_handler.py`): Multi-threaded TCP server wrapping LM clients
- `LMRequest` / `LMResponse` (`rvlm/core/comms_utils.py`): Typed request/response dataclasses
...
- State persistence via `dill` serialization to `/tmp/rvlm_state.dill`
...
See `rvlm/environments/modal_repl.py` as the canonical reference implementation.
