...
from rvlm.core.types import REPLResult, RLMIteration

if TYPE_CHECKING:
    from rvlm.environments.base_env import BaseEnv
...
def find_final_answer(text: str, environment: "BaseEnv | None" = None) -> str | None:
    ...
    if match:
        ...
        if environment is not None:
            ...
            # Detect FINAL_VAR errors (variable not found) — return None
            # so the loop continues and gives the model a chance to self-correct.
            if final_answer.startswith(f"Error: Variable '{variable_name}' not found"):
                return None
            return final_answer
        return None
