...
from rvlm.core.lm_handler import LMHandler
...
from rvlm.environments import BaseEnv, SupportsPersistence, get_environment
from rvlm.logger import RLMLogger, VerbosePrinter
from rvlm.utils.parsing import (
    find_code_blocks,
    find_final_answer,
    format_iteration,
)
...
from rvlm.utils.rvlm_utils import filter_sensitive_keys
...
class RLM:
    ...
    def __init__(
        ...
        """
        Args:
            backend: The backend to use for the rvlm.
            ...
            max_depth: The maximum depth of the rvlm. Currently, only depth 1 is supported.
            max_iterations: The maximum number of iterations of the rvlm.
            custom_system_prompt: The custom system prompt to use for the rvlm.
            ...
            logger: The logger to use for the rvlm.
            ...
            router: Optional RecursionRouter for adaptive iteration depth and early termination.
                    When provided, the router's recommended_max_iterations() overrides max_iterations,
                    and should_continue() is checked after each iteration for stall detection.
        """
        ...
        self.router = router
        # If a router is provided, let it override max_iterations with its
        # complexity-derived recommendation.  The user's explicit max_iterations
        # value acts as an absolute ceiling.
        if router is not None:
            recommended = router.recommended_max_iterations()
            self.max_iterations = min(recommended, max_iterations)
        else:
    ...
    def _setup_prompt(self, prompt: str | dict[str, Any]) -> list[dict[str, Any]]:
        """
        Setup the system prompt for the rvlm. Also include metadata about the prompt and build
