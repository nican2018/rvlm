...
def sanitize_base64_in_value(value: Any) -> Any:
    """Recursively sanitize base64 image data in a value for clean logging."""
    if value is None:
        return None
    if isinstance(value, str):
        # Replace data URIs first (more specific)
        value = BASE64_DATA_URI_PATTERN.sub("[base64 image data]", value)
        # Then replace any remaining long base64-like strings
        value = LONG_BASE64_PATTERN.sub("[base64 data]", value)
        return value
    if isinstance(value, (bool, int, float)):
        return value
    if isinstance(value, (list, tuple)):
        return [sanitize_base64_in_value(v) for v in value]
    if isinstance(value, dict):
        return {k: sanitize_base64_in_value(v) for k, v in value.items()}
    # For other types, convert to string and sanitize
    try:
        s = str(value)
        s = BASE64_DATA_URI_PATTERN.sub("[base64 image data]", s)
        s = LONG_BASE64_PATTERN.sub("[base64 data]", s)
        return s
    except Exception:
...
class RLMChatCompletion:
    ...
    def to_dict(self):
        return {
            ...
            "prompt": sanitize_base64_in_value(self.prompt),
...
class REPLResult:
    ...
    def __init__(
        ...
        rvlm_calls: list["RLMChatCompletion"] = None,
    ):
        self.stdout = stdout
        self.stderr = stderr
        self.locals = locals
        self.execution_time = execution_time
        self.rvlm_calls = rvlm_calls or []

    def __str__(self):
        return f"REPLResult(stdout={self.stdout}, stderr={self.stderr}, locals={self.locals}, execution_time={self.execution_time}, rvlm_calls={len(self.rvlm_calls)})"

    def to_dict(self):
        locals_serialized = {k: _serialize_value(v) for k, v in self.locals.items()}
        return {
            "stdout": sanitize_base64_in_value(self.stdout),
            "stderr": sanitize_base64_in_value(self.stderr),
            "locals": sanitize_base64_in_value(locals_serialized),
            "execution_time": self.execution_time,
            "rvlm_calls": [call.to_dict() for call in self.rvlm_calls],
        }
