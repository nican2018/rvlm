"""
Verbose console output for RLM / RVLM execution.

Uses a "Tokyo Night" inspired colour theme via Rich to display iteration
progress, code blocks, and final answers in the terminal.
"""
...
def sanitize_base64(text: str) -> str:
    """Replace base64 image data with a placeholder to avoid terminal spam."""
    # Replace data URIs first (more specific)
    text = BASE64_DATA_URI_PATTERN.sub("[base64 image data]", text)
    # Then replace any remaining long base64-like strings
    text = LONG_BASE64_PATTERN.sub("[base64 data]", text)
    return text


def _to_str(value: Any) -> str:
    """Convert any value to string safely, sanitizing base64 data."""
    if isinstance(value, str):
        return sanitize_base64(value)
    result = str(value)
    return sanitize_base64(result)
...
class VerbosePrinter:
    ...
    def print_code_execution(self, code_block: CodeBlock) -> None:
        ...
        if result.rvlm_calls:
            calls_text = Text()
            calls_text.append(f"\n↳ {len(result.rvlm_calls)} sub-call(s)", style=STYLE_SECONDARY)
    ...
    def print_iteration(self, iteration: RLMIteration, iteration_num: int) -> None:
        ...
        for code_block in iteration.code_blocks:
            ...
            for call in code_block.result.rvlm_calls:
                self.print_subcall(
